...
<body class="bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 antialiased">
    ...
    <div class="flex h-screen">
            ...
            <nav class="flex-grow mt-6 space-y-2">
                <template x-for="link in navigation" :key="link.name">
                    <a :href="link.href" class="flex items-center py-2.5 transition-all duration-200 relative group"
                        ...
                        <i
                            :class="[link.icon, link.active ? 'text-white' : 'text-slate-400 group-hover:text-white', 'text-xl']"></i>
        ...
        <div class="flex-1 flex flex-col transition-spacing" :class="isSidebarOpen ? 'ml-64' : 'ml-20'">
            <header
                class="bg-white dark:bg-slate-800 shadow-sm border-b border-slate-200 dark:border-slate-700 z-20 flex-shrink-0">
            ...
            return {
                ...
                navigation: [
                    ...
                    @if(auth('admin')->check() && auth('admin')->user()->role === 'superadmin')
                        { name: 'Kelola Member Admin', href: '{{ route('admin.manage.index') }}', icon: 'bi bi-person-lines-fill', active: {{ request()->routeIs('admin.manage.*') ? 'true' : 'false' }} },
                    @endif
                ],
                init() {
