...
<div class="bg-white/50 dark:bg-slate-800/50 backdrop-blur-lg rounded-2xl shadow-lg ring-1 ring-black/5 p-6 sm:p-8">
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Lengkapi detail komoditas dan petakan wilayahnya secara akurat.</p>
    </div>
    ...
    <form action="{{ route('wilayah.store') }}" method="POST" onsubmit="return cekPolygon()" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" id="nama_komoditas" name="nama_komoditas" required
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="mt-1 flex items-center gap-4">
                    <input type="color" id="colorPicker" value="#ff8c00" class="w-14 h-10 p-1 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 cursor-pointer rounded-lg">
                    <input type="text" id="colorValue" readonly class="block w-full rounded-lg border-slate-300 bg-slate-100 dark:bg-slate-900 shadow-sm dark:border-slate-600 dark:text-slate-300 cursor-not-allowed">
            ...
            <div>
                ...
                <input type="text" name="luas_wilayah" id="luasInput" readonly class="mt-1 block w-full rounded-lg border-slate-300 bg-slate-100 dark:bg-slate-900 shadow-sm dark:border-slate-600 dark:text-slate-300 cursor-not-allowed" placeholder="Akan terisi otomatis">
            </div>
        </div>
        ...
        {{-- Peta --}}
        <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Gambar Batas Wilayah Komoditas</label>
            <div id="map-wilayah-create" class="h-96 w-full rounded-lg shadow-inner bg-slate-200 dark:bg-slate-700 z-10"></div>
            <input type="hidden" name="polygon" id="polygonInput">
            <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">Pilih kecamatan terlebih dahulu sebagai acuan, lalu gambar polygon wilayah komoditas di dalamnya.</p>
        </div>

        <div class="flex justify-end pt-4 gap-4">
            <a href="{{ route('wilayah.index') }}" class="py-2 px-4 text-sm font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 rounded-lg shadow-sm border border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors">Batal</a>
            <button type="submit" class="inline-flex justify-center py-2 px-6 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors">Simpan Wilayah</button>
        </div>
    </form>
</div>
@endsection
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const map = L.map('map-wilayah-create').setView([-6.0, 139.0], 7);
    ...
    // Panggil fungsi load semua kabupaten (dari kode lama Anda)
